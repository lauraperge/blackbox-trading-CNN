...
The activation function that is mostly used throughout the model is Leaky ReLu, which is a version of the commonly used ReLu function. ReLu stands for rectified linear unit (\cite{hahnloser2000digital}) and is defined as
\begin{equation}
    \label{eq:ReLu}
    y = f(x) = \max(0,x).
\end{equation}
The popularity of the ReLu is due to it is being computationally cheap, helping faster convergence as it is linear and does not lead to the vanishing of gradients. Lastly, it promotes sparsity since all negative inputs result in zero activation which helps the model in finding meaningful features faster and prevents against overfitting. 
...
The loss function is the Kullback-Leibler divergence
The optimizer is the Stochastic gradient descent optimizer (SGD) together with Nesterov momentum

The model structure is depicted in Figure \ref{fig:CNNstruct}. It first takes the 3 input images, each of size $30 \times 30$, then we filter these 3 images with $32$ $3 \times 3 \times 3$ kernels (using zero padding to maintain image size, and the stride is 1) which gives us 32 convolutional layers of size $3 \times 30 \times 30$. It is worth to note that we use the relatively small filters due to our images being relatively small. The activation map of the layers is obtained by using Leaky ReLu, described previously in Equation \ref{eq:LeakyReLu}, preceded by batch normalization in order to prevent against overfitting. This is further supported by a dropout layer with $0.3$ drop rate. Next, we have 64 convolutional layers of size $3 \times 30 \times 30 \times$ created using the same filters, stride and zero padding as previously. This is followed by batch normalization, and then the activation map is once again made using the Leaky ReLu function. After, these convolutional layers are average pooled with $2 \times 2 \times 3$ kernels which results in 64 layers of size $3 \times 15 \times 15$ on which we again apply the Leaky ReLu followed by a dropout. The information from these layers is condensed in the following fully connected layer of size $128$ [!!!]. This is also followed by batch normalization and dropout and finally the output layer with softmax activation that gives us the probabilities for each image being "Sell", "Buy" or "Hold" class. The described model reflects the basic build of each model that has been trained and tested. Some hyperparameters and further settings were customized in the tuning phase for each model. Such aspects include the class weights (as there is an imbalance between the three classes), mini-batch sizes, dropout rates after the fully connected layer and the parameters of the optimizer.
